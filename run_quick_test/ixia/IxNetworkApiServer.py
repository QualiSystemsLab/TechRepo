...
class IxNet(object):
    ...
    def _rest_send(self, method, url, payload=None, fid=None, file_content=None):
        ...
        if str(response.status).startswith('3') is True:
            return self._rest_send(method, response.getheader('Location'), payload=payload, fid=fid, file_content=file_content)
        ...
        try:
            ...
            while int(time.time()) - start_time < int(connect_args['-connectTimeout']):
                ...
                if str(session.state).lower() == 'active':
                    ...
                    self._ws_url = 'wss://{0}/ixnetworkweb/ixnrest/ws/api/v1/sessions/{1}/ixnetwork/globals/ixnet'.format(self._base_url, sessionId)
                    options = dict({
                        'sslopt': {
